...
| 太乙 | `epoch` | which 上元積年 the count runs from: `jinjing` (《太乙金鏡式經》), `tongzong` (《太乙統宗寶鑑》) | settled by the reading — phase 20 |
| 太乙 | `ji` | which register: `nianji` (年計), later `yueji` · `riji` · `shiji` | `nianji` |
| 太乙 | `yearBoundary` | where the counted year begins: `lichun` (立春), `dongzhi` (冬至), `chunjie` (正月初一) | settled by the reading — phase 20 |

**太乙's `epoch` is not a row like the others and the difference is the whole
...
the standard for anything underivable, and whether this quantity can meet it
is unknown at the time of writing, which is why that phase is gated on a
reading rather than scheduled as work. The three rows are provisional in the
way `gong` was provisional before the board existed.
...
content.** That registry answers «what can a consultation be laid on» — four
rows, `needs: 'question' | 'birth'`. This page asks «what does this engine
compute», and the two sets already differ: the 曆注 are computed and are not an
instrument, the almanac layer being a page of a published book rather than a
board. Phase 20 widens the gap on purpose, its board being an address and not
an instrument. So this page reads a registry of its own — the boards, the
...
### Phase 20 — 太乙神數, and the reading that gates it

**Planned, and the first step is not code.** The third of the 三式 gets a
board and a section. It is the first thing here whose subject is neither a
question nor a person: 太乙主天, and what this board is laid on is a year.

**The reading comes before the code, and the phase may legitimately end in
it.** 《太乙金鏡式經》 (王希明, 唐, c. 730, 十卷) is in the 四庫全書 and on
Wikisource, and this repository has read it once already — `purposes.ts` and
the 八門 section of `docs/sources.md` lean on 卷二 for the domains of the
gates. It is read again against three questions, and all three answer before a
line is written:
...
If the first fails, § "The standard, stated once" of `docs/sources.md` already
says what happens: the entry is left out and the absence is written down.
**That outcome is a delivery and not a failure of the phase.** This repository
writes its absences — the 年命 doctrine, 紫氣's missing referent, every "What
is not here" — and a 太乙 section naming the epochs the texts give and why none
could be preferred is worth more than a board rotated by an unstated constant.
The symmetry of the 三式 buys nothing: a board earns its place by having a
procedure a source states, never by filling a hole in a catalogue, and this is
the phase where that sentence meets the case it was written for.

**Everything below assumes the three answered.**
...
sentinel at `apps/web/test/load.test.ts:163` keeps its assertion —
`?instrument=taiyi` still falls back to `qimen`, because it is still not an
instrument — but **its comment stops being true and is rewritten**: what is
...
sentinel is not written to fail on the day the board lands.
...
**The verification has no runnable reference, and the substitute is named in
advance.** Nothing open computes 太乙, and the closed programs that do disagree
with each other, so the check is the **worked historical boards the texts
carry** — the tradition auditing itself — transcribed into fixtures. That is
weaker evidence than `lunar-javascript` was for the pillars, in the way the
七政四餘 frame's over-determination was, and it is written down as such at the
surface. **Those boards are also the section's content**, and the two uses do
not conflict so long as the years the instrument reads badly are shown beside
the years it reads well. A page displaying only the hits is advertising.
...
**The surfaces to cross**, per the `new-feature` skill: the calculation in
`core` with its tests, both catalogs, `format.ts`, `qimen taiyi`,
`compute_taiyi`, `/api/taiyi` with `/text` and `/plate`, the drawing, the
section at `/[lang]/taiyi`, the README, `docs/sources.md` and
`docs/agent-prompt.md`. No `/prompt`, per above.
...
**What does not move.** The 年命 rule and `purposes.ts`; one board to a prompt;
...
top of this phase exists to hold.
