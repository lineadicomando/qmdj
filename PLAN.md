...
> There is deliberately **no `/prompt`**: the twelve 宮 of this board are the
> ring a 六壬 board's 月將 is seated on, so handing a model both would be one
> fact counted twice — the rule in `CLAUDE.md`, sharpened.
